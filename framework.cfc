...
*/
	...
	/*
	 *	buildFullURL() should be used from views to construct urls for ajax requests
	 */
	public string function buildFullURL( string action = '.', string path = variables.magicBaseURL, any queryString = '' ) {
		return getMyURL() & buildURL( arguments.action, arguments.path, arguments.queryString);
	}
	...
	/*
	 * returns the real base URL for redirects and links etc
	 */
	public string function getMyURL() {
		var securedURL = '';
		var myPort = '';
		if (cgi.server_port_secure){securedURL = 's';};
		if (!listFind('80,443',cgi.server_port)){myPort = ':'&cgi.server_port;};
		var baseURL = 'http' & securedURL & '://' & cgi.server_name & myPort;
		return baseURL;
	}
